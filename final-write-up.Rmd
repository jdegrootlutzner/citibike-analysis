...
We wrote code to wrangle the data so that it is ready to make interesting graphs about each station but we never had time to make the graphs. The code can be found in station-analysis.Rmd.
...
```
16.86 is too high of a Root Mean Squared Error to be effective but just for fun let's look at how accurate the model was at classifying if the difference was positive or negative (export or import).
```{r, cache = TRUE, echo=FALSE}
...
```
Does our model do better than predicting all of the differences as negative ?
```{r, cache = TRUE, echo=FALSE}
all_true <- (prediction < -10000)
confusionMatrix(all_true, actual_positive_change)
```
Our model did do better than predicting all of the differences as positive. But without an accurate number this does not help us too much. Let's compare this result to SVM .
